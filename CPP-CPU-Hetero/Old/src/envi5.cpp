...
Lpoint findValidMin(Octree octree, Vector3D* boxMin, Vector3D* boxMax, int* numInside)
{
    ...
    if(isLeaf(octree))
    {
        ...
        uint64_t size = octree->vpoints.size();
        if(size != 0)
        {
            ...
            for (i = 0; i < size; i++) {
              // if(insideBox2D(octree->points[i], *boxMin, *boxMax))
                if(insideBox2D(octree->vpoints[i], *boxMin, *boxMax))
                {
                  // tmp = *(octree->points[i]);
                  tmp = *(octree->vpoints[i]);
                    if (tmp.z < min.z) {
                        min = tmp;
                    }
                    (*numInside)++;
...
uOctree createOctreeF(Vector3D center, float radius)
{
    ...
    uOctree oct( new nIntermedio(center, radius) );
...
void createOctantsF(Octree oct)
{
    ...
    // {
      for(i = 0; i < 8; i++)
      {
          ...
          oct->octants[i] = uOctree{ new nIntermedio(newCenter, oct->radius * 0.5) };
      }
    // }
}
...
void insertPointF(Lpoint *point, Octree octree, float minRadius, int nivel)
{
    ...
    if(isLeaf(octree))
    {
        ...
        {
          // octree->reset( new nIntermedio(octree->center, octree->radius) );
          createOctantsF(octree);
          // fillOctants(octree);
          idx = octantIdx(point, octree);
          insertPointF(point, (octree->octants[idx]).get(), minRadius, nivel+1);
...
void deleteOctree(Octree octree)
{
    ...
    if(isLeaf(octree))
    {
        ...
        for(i = 0; i < 8; i++) {
            ...
            (octree->octants[i]).reset(); // free memory for unique_ptr
        }
...
void prueba1( uOctree& octree )
{
  printf("myfloat %g\n", (octree)->radius);
  octree.reset( new nIntermedio({0.0,0.0,0.0}, 33.0) );
  // printf("myfloat %g\n", (octree)->radius);
  // octree.reset();
  return;
}
