...
//This version avoids traversing the tree always from root to leaves
//The tree (quadtree although named Octree) now includes pointers from nodes to parents that can be traversed bottom-up
//This optimization does not pay off due to increasing the memory footprint (extra pointer in all the nodes)
