...
int main( int argc, char* argv[]) {
 ...
 // Ficheros
    ...
    // std::unique_ptr<Octree_t> octreeIn;
    // std::unique_ptr<Octree_t> grid;
    Octree octreeIn;
    Octree grid;
    ...
    // std::unique_ptr<Octree_t> ooct( new Octree_t() );
    //
    // ooct->numPts = 0;
    //
    // free(ooct.get());

    // ooct = (Octree_t*)malloc(sizeof(Octree_t));
    ...
    while(bucle_entrada){
        ...
        printf("STAGE 1 ");
        // sleep(1);
        countMin = stage1cpp(Wsize, Overlap, Crow, Ccol, minNumPoints, minIDs, octreeIn, min);
        ...
        if(Bsize > 0){
            ...
            addMin = stage3cpp(Bsize, Crowg, Ccolg, minGridIDs, octreeIn, grid, min);
