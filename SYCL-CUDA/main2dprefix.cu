/*
Si quiero 10 puntos, con 2 puntos por nodo hoja (El nombre del mapa no se usa, pero hay que ponerlo):
./debug2dprefix.out data/INAER_2011_Alcoy.xyz 2 16 16 10
*/
...
__device__
inline uint32_t morton2D(real_v xy , const real_t resolution = 65536.0 ) noexcept
{
...
/*Con este método aprovecho que estoy haciendo "m64[i] = uint64_t(i)" para
simplificar el problema y no utilizar un vector de nuevos morton*/
__global__
void init_leafs_size3(node_t* n, aabb_t* bb, const point_t* p, const uint32_t* idx,
  const int num_objects, const int r_objects, const int chunkDim, const aabb_t default_aabb,
  const aabb_t BBox, const real_v diffBox)
{
  const uint32_t index = threadIdx.x + blockIdx.x * blockDim.x;
...
__global__
void init_nodes5( node_t* n, const uint32_t num_leaves, uint32_t* prefixes )
{
  const uint32_t i = threadIdx.x + blockIdx.x * blockDim.x;
  ...
  /*Como estoy haciendo "m64[i]=i", el código es el propio índice.
  Por tanto no necesito el vector "node_code"*/
  ...
  } while( 0 <= i_tmp && i_tmp < num_leaves && delta_min < ::__clzll(self_code ^ i_tmp) );
  ...
  {
    i_tmp = idx + (l + t) * d;
    if( 0 <= i_tmp && i_tmp < num_leaves && delta_min < ::__clzll(self_code ^ i_tmp) )
      l += t;
  }
  ...
  if(idx == gamma){
    // prefixes[lidx] = ::__clzll(lidx ^ lidx);
    prefixes[lidx] = 64;
  }
  if(jdx == gamma+1){
    // prefixes[ridx] = ::__clzll(ridx ^ ridx);
    prefixes[ridx] = 64;
  }
...
int main(int argc, char* argv[])
{
  ...
  std::uniform_real_distribution<real_t> unix(BBox.lower.x, BBox.upper.x);
  std::uniform_real_distribution<real_t> uniy(BBox.lower.y, BBox.upper.y);
  std::uniform_real_distribution<real_t> uniz(0.0, 10.0);
  ...
  cudaMallocManaged(&prefixes, r_num_nodes*sizeof(uint32_t));
  ...
  cudaMemPrefetchAsync(prefixes, r_num_nodes*sizeof(uint32_t), deviceId);
  ...
  int i = 0;
  for(; i<num_objects; i++){
    point_cloud[i].x = (int)unix(mt);
    point_cloud[i].y = (int)uniy(mt);
    point_cloud[i].z = (int)uniz(mt);
  }
  point_cloud[9].z = -1.0;
#else
...
// #endif
  ...
  init_leafs_size3<<<numberOfBlocks, threadsPerBlock>>>( &node_listS[r_num_internal_nodes], 
    &aabb_listS[r_num_internal_nodes], 
    point_cloud, 
    indices_out,
    num_objects,
  ...
  init_nodes5<<<nBlocks_node_func, threadsPerBlock>>>( node_listS, r_num_objects, prefixes );
...
// mortonError = cudaGetLastError();
  ...
  std::cout << std::endl;
  for(int i=0; i<r_num_nodes; i++){
    std::cout << prefixes[i] - prefixes[0] << " ";
  }
  std::cout << std::endl;
#endif
