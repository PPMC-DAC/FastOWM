...
int insideBox(aabb_t& global, const point_t& point);
...
uint32_t stage2CPU(uint32_t countMin, uint32_t* minIDs){
  ...
  for( jj=0 ;  minIDs[jj] == 0xFFFFFFFFu ; ++jj );
...
int save_file(std::string file_name, const uint32_t* ids, const uint32_t countMin, const point_t* pointer)
{
  ...
  for( int i=0; i<countMin; i++){
    out << pointer[ids[i]].x << " " << pointer[ids[i]].y << " " << pointer[ids[i]].z << std::endl;
...
/* Saves results in .xyz format */
int save_file_append(std::string file_name, const uint32_t* ids, const uint32_t countMin, const point_t* pointer)
{
  // int internalCount = 0;
  std::ofstream out;
  out.precision(std::numeric_limits<double>::digits10);
...
int save_time(std::string file_name, std::string map_name, uint32_t chunkDim, uint32_t threadsPerBlock,
  uint32_t numberOfBlocks, double build_time, double traverse_time, float error)
{
  // uint32_t size = pointList.size();
  ...
  if(!out.is_open())
    return -1;
// chunkDim is probably MaxNumber in the paper
  out << map_name << " " << chunkDim << " " << threadsPerBlock << " ";
  out << numberOfBlocks << " " << build_time << " " << traverse_time << " " << error << std::endl;
...
inline void createAABB(const point_t& p, const float radio, aabb_t& box){
  box.lower.x = p.x - radio;
  box.lower.y = p.y - radio;
  box.upper.x = p.x + radio;
  box.upper.y = p.y + radio;
  return;
}

/* Check results by comparing with GOLD result */
double check_results(std::string filename, std::vector<uint32_t>& ids, point_t* pointer, float Displace)
{
  std::string line;
  // char delim;
  point_t aux;
  uint32_t npoints = 0;
  std::vector<point_t> vgold, v;
  ...
  std::sort(vgold.begin(), vgold.end(), [](const point_t a, const point_t b){
    return a.z < b.z;
  });
  ...
  std::sort(v.begin(), v.end(), [](const point_t a, const point_t b){
    return a.z < b.z;
  });
  ...
  aabb_t box;
  for(auto p : v){
    createAABB(p, Displace*0.5, box);
    for (auto it = vgold.begin(); it != vgold.end(); ++it){
    // for(auto pg : vgold){
      if(insideBox(box, *it)){
...
std::string obtain_gold( std::string inputTXT)
{
  if( inputTXT.find("INAER_2011_Alcoy.xyz") != std::string::npos ){
    return "data/INAER_2011_Alcoy_salidaGOLD.xyz";

  } else if( inputTXT.find("INAER_2011_Alcoy_Core.xyz") != std::string::npos ){ // Alcoy
    return "data/INAER_2011_Alcoy_Core_salidaGOLD.xyz";

  } else if( inputTXT.find("BABCOCK_2017_Arzua_3B.xyz") != std::string::npos ){ //Arzua
    return "data/BABCOCK_2017_Arzua_3B_salidaGOLD.xyz";

  } else if( inputTXT.find("V21_group1_densified_point_cloud.xyz") != std::string::npos ){ //Brion forestal
    return "data/V21_group1_densified_point_cloud_salidaGOLD.xyz";

  } else if( inputTXT.find("V19_group1_densified_point_cloud.xyz") != std::string::npos ){ //Brion urban
    return "data/V19_group1_densified_point_cloud_salidaGOLD.xyz";
...
void map_file_atof_tbb_th(std::string fname, acc_t cloud, uint32_t num_objects)
{
    size_t length = 0u;
...
aabb_t merge(const aabb_t& lhs, const aabb_t& rhs) noexcept
{
    aabb_t merged;
...
__device__ __forceinline__
real_v getCenter(const aabb_t& box) noexcept
{
    real_v c;
...
int boxOverlap2D(const aabb_t& global, const aabb_t& local)
{
  ...
  if((local.upper.x < global.lower.x ) ||
    (local.upper.y < global.lower.y ))
      return 0;

  if((local.lower.x > global.upper.x ) ||
    (local.lower.y > global.upper.y ))
      return 0;
...
int isAllOverlaped(const aabb_t& global, const aabb_t& local)
{
  if((local.upper.x > global.upper.x ) ||
    (local.upper.y > global.upper.y ))
      return 0;

  if((local.lower.x < global.lower.x ) ||
    (local.lower.y < global.lower.y ))
      return 0;
...
int insideBox(aabb_t& global, const point_t& point)
{
    if((point.x > global.lower.x ) && (point.y > global.lower.y ))
    {
        if((point.x < global.upper.x ) && (point.y < global.upper.y ))
        {
            return 1;
        }
    }
...
struct Split_task
{
  __host__ __device__ Split_task() {}
  __host__ __device__ Split_task(const uint32_t id, const uint32_t in, const uint32_t n, const uint32_t min) : 
