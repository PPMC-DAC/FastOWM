...
void init_leafs_size(node_t* n, aabb_t* bb, const point_t* p, const uint32_t* idx,
  const int num_objects, const int r_objects, const int chunkDim)
{
  int index = threadIdx.x + blockIdx.x * blockDim.x;
  int stride = blockDim.x * gridDim.x;
  ...
  for(int i = index; i<r_objects; i += stride)
  {
    ...
    uint32_t id = idx[start]; // obtengo el índice del primer punto del chunk

    n[i].object_idx = id; // modifico el índice del nodo hoja

    point_t auxp = p[id]; // obtengo el punto
    aabb_t auxbb = {{auxp.x,auxp.y},{auxp.x,auxp.y}};

    for(int j=start+1; j<end && j<num_objects; j++) {
      auxp = p[idx[j]]; // obtengo el punto
      auxbb = merge(auxbb, {{auxp.x,auxp.y},{auxp.x,auxp.y}});
      // bb[i] = merge(bb[i],auxbb);
    }
    bb[i] = auxbb;
  }
...
__global__
void create_aabb_size( const node_t* n, aabb_t* bb, int* flags, 
      const size_t num_internal_nodes, const size_t num_nodes)
{
  uint32_t index = num_internal_nodes + threadIdx.x + blockIdx.x * blockDim.x;
  uint32_t stride = blockDim.x * gridDim.x;
...
int main(int argc, char* argv[])
{
  ...
  int chunkDim = 16;
  int r_num_objects = (int)(num_objects/chunkDim);
  int r_num_internal_nodes = r_num_objects - 1;
  int r_num_nodes = 2*r_num_objects -1;
  ...
  cudaMallocManaged(&flagsS, r_num_objects*sizeof(int));
  ...
  cudaMemPrefetchAsync(flagsS, r_num_objects*sizeof(int), deviceId);
...
// #endif
        ...
        // node_listS[idx].left_idx  = gamma;
        // node_listS[idx].right_idx = gamma + 1;
        uint32_t right_idx = left_idx + 1;
...
// mortonError = cudaGetLastError();
  ...
  uint32_t nBlocks_aabb_func = (uint32_t)(r_num_objects/threadsPerBlock) + 1;
