...
Qtree parallel_qtree_pf2( int level, Vector2D center, float radius, delimiter_t node_delimiter, Lpoint* point_cloud, uint Npoints )
{
  ...
  tbb:: parallel_for( tbb::blocked_range<int>{0, static_cast<int>(Npoints)},
                      [root,point_cloud](tbb::blocked_range<int> r ) {
...
//         // newmin = searchNeighborsMin(&cellCenter, qtreeIn, Wsize/2, &cellPoints);
//         // newmin = cpuSearchNeighborsMin(cellCenter, qtreeIn, Wsize/2, cellPoints);
//         Lpoint newmin = gpuSearchNeighborsMin(cellCenter, qtreeIn, Wsize*0.5, cellPoints);
//         // newmin = gpuSearchIndex(cellCenter, p_arbol, p_puntos, Wsize*0.5, Wsize*0.5, cellPoints);
...
/* Reads the point cloud in .xyz format */
int read_pointsC(std::string file_name, Lpoint* point_cloud, uint Npoints)
{
  FILE* fileLAS;
...
//     gpu_queue.parallel_for<class minSearch>(range<2>(Ccol,Crow), [=](id<2> index) { 
...
//     gpu_queue.parallel_for<class minSearch>(range<2>(nRows, nCols), [=](cl::sycl::id<2> index) { 
...
//       gpu_queue.parallel_for<class minSearch>(range<2>(chunk, Crow), [=](cl::sycl::id<2> index) { 
...
//     // sycl::event e1 = gpu_queue.parallel_for<class minSearch>(range<2>(chunk, nCols), [=](cl::sycl::id<2> index) { 
