...
// __device__ __host__
// inline std::uint32_t morton_code(float4 xyz) noexcept
// {
//     xyz.x = min(max(xyz.x * 1024.0f, 0.0f), 1023.0f);
//     xyz.y = min(max(xyz.y * 1024.0f, 0.0f), 1023.0f);
//     xyz.z = min(max(xyz.z * 1024.0f, 0.0f), 1023.0f);
//     const std::uint32_t xx = expand_bits(xyz.x);
//     const std::uint32_t yy = expand_bits(xyz.y);
//     const std::uint32_t zz = expand_bits(xyz.z);
//     return xx * 4 + yy * 2 + zz;
// }
...
{
    /* count leading zeros; el operador ^ realiza un xor bit a bit; por tanto, lo que se consigue
    con esto es comparar los dos índices y ver donde está la primera diferencia */
