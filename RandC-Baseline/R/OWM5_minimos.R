  ...
  OWM <- function(PlotPts,WSize,BSize,Overlap){
    ...
    for (j in c(Despl.Count:0)) {
      ...
      for (i in c(Despl.Count:0)) {
        ...
        # extent     : 513738.5, 513858.5, 5403116, 5403188  (xmin, xmax, ymin, ymax)
        ...
        # cat(sum(cells==59, na.rm=TRUE))

        # En cuantas se cumple la condicion?
        # cat("Alguna menor que 298??\n")
        # cat(c(which(sort(table(cells)) < 298)),'\n')
        ...
        print(PlotPts@data[, .I[which.min(Z)], by = cells]$V1) # row number in PlotPts@data corresponding to each cell
        ...
        cat(cells[PlotPts@data[, .I[which.min(Z)], by = cells]$V1],'\n\n')
        cat("Es decir, estoy contando la celda",cells[979],'\n')
        # print(PlotPts@data[, .I[which.min(Z)], by = cells]$V1)
        rm(layout,cells)
      }
    }
    cat("\n///////////////////////////FIN BUCLE///////////////////////////\n")
