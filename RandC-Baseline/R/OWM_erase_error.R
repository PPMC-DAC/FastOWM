...
# 6.Function definitions --------------------------------------------------
  OWM <- function(PlotPts,WSize,BSize,Overlap){
    ...
    cat("\nNumero de puntos: ", PlotPts@header@PHB$`Number of point records`, "\n\n")
    ...
    for (j in c(Despl.Count:0)) {
      ...
      for (i in c(Despl.Count:0)) {
        ...
        cat("\nAlguna menor que 290??\n")
        cat(c(which(sort(table(cells)) < 290)),'\n')
        # cells[cells %in% c(which(sort(table(cells)) < min.Count.Points))] <- NA
        cells[cells %in% c(which(sort(table(cells)) < 290))] <- NA
