...
static const int REALLOC_INCREMENT = 128;
...
unsigned int stage1(unsigned short Wsize, double Overlap, unsigned short Crow, unsigned short Ccol,
  ...
  unsigned int countMin = 0, cellPoints = 0, idmin = 999;
...
unsigned int stage3(unsigned short Bsize, unsigned short Crow, unsigned short Ccol,
    ...
    unsigned int addMin = 0, cellPoints = 0, idmin = 999;
...
Lpoint** neighbors2Dmin(Lpoint *point, Vector3D boxMin, Vector3D boxMax, Octree octree, Lpoint **ptsInside, int *ptsInside_size, int *numInside, int* idmin)
{
    int i = 0;
        ...
        if(!isEmpty(octree))
        {
            for(i = 0; i < octree->numPts; i++)
            {
                if(insideBox2D(octree->points[i], boxMin, boxMax) && point->id != octree->points[i]->id)
                {
                    ...
                    ptsInside[(*numInside)] = octree->points[i];
                    (*numInside)++;
                    if(octree->points[i]->z < round2d(ptsInside[*idmin]->z)){
                        *idmin = (*numInside)-1;
                        // printf("Hay un minimo\n");
                    }
                }
            }
        }
    ...
    else
    {
        for(i = 0; i < 8; i++)
        {
            ...
            {
                ptsInside = neighbors2Dmin(point, boxMin, boxMax, octree->octants[i], ptsInside, ptsInside_size, numInside, idmin);
            }
        }
    }
...
Lpoint** searchNeighbors2Dmin(Lpoint *point, Octree octree, float radius, int *numNeighs, int* idmin)
{
    Vector3D boxMin, boxMax;
    Lpoint **ptsInside = NULL;
    int ptsInside_size = 0;
    ...
    *idmin = 0;
    makeBox(point, radius, &boxMin, &boxMax);
    ptsInside = neighbors2Dmin(point, boxMin, boxMax, octree, ptsInside, &ptsInside_size, numNeighs, idmin);
