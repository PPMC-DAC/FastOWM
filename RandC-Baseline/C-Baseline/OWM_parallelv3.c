...
int main( int argc, char* argv[]){
  ...
  if(Overlap != 0){
      ...
      // int i1;
      // #pragma omp parallel for firstprivate(minIDs,searcher) shared(index,vMin) schedule(dynamic,block_size)
      // for(int i1=omp_get_thread_num(); i1<countMin; i1++){
      ...
      int i2;
      // #pragma omp parallel for firstprivate(minIDs,vMin) shared(searcher) schedule(dynamic,block_size)
      for( i2=0; i2<index; i2++){
        if(vMin[i2].count > 1){
          minIDs[searcher] = vMin[i2].id;
          searcher++;
        }
      }
