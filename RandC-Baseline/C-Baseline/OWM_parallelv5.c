...
int main( int argc, char* argv[]){
    ...
    #pragma omp parallel shared(countMin) firstprivate(Ccol,Crow,minIDs,octreeIn,Wsize,Overlap,Displace,initX,initY) \
  ...
  if(Overlap != 0){
      ...
      searcher=0;
      // #pragma omp parallel for firstprivate(vMin,index) shared(searcher) schedule(dynamic,block_size)
