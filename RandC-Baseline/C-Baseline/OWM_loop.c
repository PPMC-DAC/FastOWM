...
#include "landing_v3_plane.h"
#include "landing_approximation.h"

#define EPS 1e-10

int main( int argc, char* argv[]){

  //Utilizo la que está definida en utils/point.h
  // typedef struct{
  //   unsigned int id;
  //   double x;
  //   double y;
  ...
  // Abro el fichero
  FILE* fileLAS;
  // if((fileLAS = fopen("../datos/sample54.xyz","r")) == NULL){
  ...
  for(int i=0; i<Npoints ; i++){
    //Obtengo los datos id X Y Z
    pointer[i] = createPoint(i, 0.0, 0.0, 0.0, 0, zeroArray);
    // pointer[i].id = i;
  ...
  for(int i = 0; i < Npoints; i++)
     ...
     insertPointMinRadius(&pointer[i], octreeInit, 0.0);
  // insertPoints(pointer,Npoints,octreeInit);
  ...
  while(cellCenter.y < max.y){
      ...
      while(cellCenter.x < max.x){
          ...
          if(cellPoints >= minNumPoints ){
              ...
              // insertPoints(*neighbors,cellPoints,cell);
              for(int i = 0; i < cellPoints; i++)
            	   insertPointMinRadius(neighbors[i], cell, 0.0);
              // cell->numPts = cellPoints;
              // printf("NUmeor de puntos del octree: %d\n", cell->numPts );
