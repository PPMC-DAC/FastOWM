...
int main( int argc, char* argv[]){
  ...
  LASPointH p = NULL;
  reader = LASReader_Create("../datos/sample54.las");
  ...
  // reader_m = LASReader_Create("../datos/minimos.las");
  // header_m = LASReader_GetHeader(reader_m);
  ...
  while(yymax<ymax){
      ...
      loopX=1;
      while(xxmax<xmax){
          sprintf(command,"las2las -i ../datos/sample54.las -o ../datos/window.las --minx %f --miny %f --maxx %f --maxy %f",xxmin,yymin,xxmax,yymax);
          // sprintf(command,"las2las -i ../datos/INAER_2011_Alcoy.las -o ../datos/window.las -e \"%f %f %f %f\"",xxmin,yymin,xxmax,yymax);
          if(system(command) < 0){
            perror("system");
            return 1;
          }
          ...
          if(cellPoints >= minNumPoints ){
              p = LASReader_GetNextPoint(reader);
              zzmin=round(LASHeader_GetMinZ(header)*100)/100;
              ...
              //Solo necesito hacer esa comprobacion porque no va a haber ninguno mas pequeño
              //Cuando la condicion se cumpla es seguro el minimo
              while(round(LASPoint_GetZ(p)*100)/100 > zzmin){
              // while(abs(LASPoint_GetZ(p)-zzmin) > 0.00000001f){
