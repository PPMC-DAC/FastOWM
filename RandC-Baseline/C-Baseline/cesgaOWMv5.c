...
// int findMinRec(int A[], int* idmin, int n)
// {
//
//     int min;
//
...
// }
//
// double findMinRecD(double* A, int* idmin, int n)
// {
//     double zmin;
//
//     if(n==1)
...
int cmpLpoint (const void* a, const void* b){
   ...
   return 100*( round( ((Lpoint*)a)->z * 100)/100 - round( ((Lpoint*)b)->z * 100)/100 );
...
int cmpLpoint2 (const void* a, const void* b){
   // Le pongo el 100 para que se quede con el signo
                ...
                // {
                    ...
                    for( ii=0 ; ii < Crow ; ii++ ){
                        ...
                        if(cellPoints >= minNumPoints ){
                            ...
                            // block_min = malloc(cellPoints*sizeof(zSearch));
                            // for(int i=0; i<cellPoints; i++){
                            //     block_min[i].id=neighbors[i]->id;
                            //     block_min[i].z=neighbors[i]->z;
                            // }
                            // qsort(block_min,cellPoints,sizeof(zSearch),&cmpLpoint2);
                            // idmin = block_min[0].id;
                            // for(int i=0; i<cellPoints; i++) printf(" %.2f ", block_min[i].z); printf("\n\n");
                            ...
                            #pragma omp critical
                            {
                                ...
                                // minIDs[countMin] = block_min[0].id;
                                countMin++;
                            }
