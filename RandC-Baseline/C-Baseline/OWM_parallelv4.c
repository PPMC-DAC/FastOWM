...
int main( int argc, char* argv[]){
    ...
    // for( int ii = 0 ; cellCenter.y + Wsize/2 - Wfactor*Wsize < max.y + Wsize*Overlap ; cellCenter.y += Displace ){
        ...
        // cellCenter.x = initX;
        #pragma omp parallel for shared(countMin) firstprivate(cellCenter,minIDs,octreeIn,Wsize,Overlap,Displace,initX,initY) \
                                  private(neighbors,cellPoints,idmin) schedule(dynamic,block_size)
        for( ii = 0 ; ii < Crow ; ii++ ){
  ...
  if(Overlap != 0){
      ...
      printf("/////////////////////////// MIN SELECT ///////////////////////////\n\n");
      searcher=0;
      // #pragma omp parallel for firstprivate(minIDs,searcher) shared(index,vMin) schedule(dynamic,block_size)
      for(int i=0; i<countMin; i++){
          ...
          while(minIDs[i]!=vMin[searcher].id && searcher<index)
            searcher++;
      ...
      // int i1;
      // // #pragma omp parallel firstprivate(minIDs,searcher,countMin) private(i1) shared(vMin)
      // // {
      //     for(i1=0; i1<countMin; i1++){
      //         // Voy a recorrer todo el bucle, pero al parelelizar será más rápido
      //         // for(searcher=0; searcher<index; searcher++){
      //
      //         for(searcher=omp_get_thread_num(); searcher<countMin; searcher+=num_procs){
      //           if(minIDs[i1]==vMin[searcher].id) {
      //             // #pragma omp atomic
      //             vMin[searcher].count++;
      //             // printf("Acumulo el %d; Posicion %d\n", minIDs[i], searcher);
      //             searcher=countMin+1; //esto es como un break
      //             index++; //Cuento aquí para no tener en cuenta los que no se repiten
      //           }
      //         }
      //
      //         if(searcher==countMin) {
      //           // printf("NUEVO: %d; Pospragma omp atomicicion %d\n", minIDs[i], index);
      //           vMin[i1].id = minIDs[i1];
      //           vMin[i1].count = 1;
      //           // index++;// para llevar la cuenta
      //         }
      //     }
      // // }
      ...
      /*Se el cuantos he agregado(index), pero no donde están*/
      // int i2;
      #pragma omp parallel for firstprivate(vMin) shared(searcher) schedule(dynamic,block_size)
