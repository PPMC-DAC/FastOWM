// This version differs from optim2 in the following ways:
// 1.- Adds memoization feature to avoid computations that have already been done (See paper Sec. 3.3)
// 2.- Stage1 remember the minimum of the previous step and if it is inside the new SW in the next step it can take advantage
// 3.- The tree stores the minimum and number of points hangin from each node. This avoids finding the minimum traversing a 
//     node that is fully overlaped by the SW
