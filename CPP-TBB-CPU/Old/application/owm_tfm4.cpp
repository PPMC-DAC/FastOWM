...
int main( int argc, char* argv[]) {
 ...
 // Ficheros
    ...
    // std::vector<std::unique_ptr<Node_t>> tareas;
    //
    // tareas.push_back(std::unique_ptr<Node_t>{ new nHoja });
    // tareas.push_back(std::unique_ptr<Node_t>{ new nIntermedio });
    ...
    for(int i = 0; i < Npoints; i++){
      ...
      insertPointF(&pointer[i], octreeIn.get(), minRadius, 0);
      // printf("punto %d\n", i);
    }
        ...
        if(Bsize > 0){
            ...
            if(Overlap != 0.0){
              for(int i = 0; i < searcher; i++)
                insertPointF(&pointer[minIDs[i]], grid.get(), minRadius, 0);
            } else { // because data is all out of order; NO stage2
              for(int i = 0; i < Ncells; i++)
                if(minIDs[i] >= 0) insertPointF(&pointer[minIDs[i]], grid.get(), minRadius, 0);
            }
