...
Octree_t::Octree_t(Octree oct, Vector2D c, float r) : parent(oct), center(c), radius(r) {
  for(int i = 0; i < 4; i++)
    octants[i] = NULL;
}
...
Lpoint searchNeighborsMinParent(Vector2D* point, Octree& parent, float radius, int* numNeighs)
{
    Vector2D boxMin, boxMax;
    Octree actual_parent = parent;

    *numNeighs = 0;
    makeBox(point, radius, &boxMin, &boxMax);

    while(actual_parent->parent != NULL && !boxTotalOverlap2D(boxMin, boxMax, actual_parent)){
    // while(actual_parent->parent != NULL){
      actual_parent = actual_parent->parent;
    }

    // if(actual_parent->parent != NULL) {
    //   // printf("my size: %zu\n", (*parent)->points.size());
    //   actual_parent = actual_parent->parent;
    ...
    return findValidMinParent(actual_parent, &boxMin, &boxMax, numNeighs, parent);
}
